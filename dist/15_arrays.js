...
let names2 = ['Danny', 'Samantha'];
// If you try to assign an array of numbers to a variable declared as string[], you get an error:
let incorrectNames = [1, 'Danny', 'Samantha']; // Type 'number' is not assignable to type 'string'.
...
let multiDimensionalArray = [['Danny', 'Samantha'], ['Smith', 'Dakota'], 'Hello']; // Type 'string' is not assignable to type 'string[]'.
// This works for any number of multidimensional arrays:
let superMultiDimensionalArray = [[[3], [4]], [[1], [2]], [5]]; // Errors out on the last index because it was expecting [5] inside the purple brackets, not the number 5: Type 'number' is not assignable to type 'number[]'.
